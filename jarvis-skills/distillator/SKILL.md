---
name: jarvis-distillator
description: JARVIS 分支 Skill。无损文档蒸馏压缩，将 JARVIS 索引的文档压缩为 Agent 可高效消费的精简版。是 JARVIS "索引不倾倒"原则的工具化实现。支持语义拆分、JARVIS 技能蒸馏模式和往返验证。
---

# Jarvis Distillator — 文档蒸馏引擎

JARVIS 的分支 Skill，是 JARVIS 工作原则 **"索引不倾倒"** 的工具化实现。将 document-project 扫描生成的大量文档压缩为 Agent 可高效消费的精简版，**无损压缩而非摘要**。
...
| --jarvis-mode | 否 | JARVIS 技能蒸馏模式（见下方说明） |
...
`--quick-ref` 与 `--jarvis-mode` 正交：
- `--jarvis-mode` 控制**为谁蒸馏**（下游消费者身份）
- `--quick-ref` 控制**输出多细**（输出粒度）
- 两者可组合：`--quick-ref --jarvis-mode=repo-skill` = 为 Repo Skill 生成速查卡版本
...
### JARVIS 技能蒸馏模式（--jarvis-mode）

当指定 `--jarvis-mode` 时，自动根据目标技能类型调整蒸馏策略：

| 模式值 | downstream_consumer 等效 | 保留重点 | 去除重点 |
|---|---|---|---|
| `repo-skill` | jarvis-repo-skill | 接口契约、数据模型、构建命令、关键约束 | 环境搭建细节、迁移历史、所有 TODO |
| `workflow-skill` | jarvis-workflow-skill | 业务流程、阶段定义、集成点、成功标准 | 代码实现细节、文件路径 |
| `source-skill` | jarvis-source-skill | 访问路径、搜索策略、路由规则、Owner | 具体内容、配置细节 |
| `entry` | jarvis-entry | 路由表、快速参考、模块索引 | 所有详细内容 |

JARVIS 模式下的额外规则：
- 源文件路径必须是相对路径（保证蒸馏版可移植）
- 每个分片开头必须包含 "本分片对应 JARVIS 技能：{类型}"
- 不确定的内容保留但标注 `[未确认]`，不删除
...
2. distillator --jarvis-mode=repo-skill → 蒸馏版（按主题拆分）  ← Layer 2
