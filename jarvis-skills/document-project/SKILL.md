---
name: jarvis-document-project
description: JARVIS 分支 Skill。为无文档的老项目（棕色地带项目）生成 AI 可消费的文档骨架。扫描代码提取业务模型、接口信息、数据架构、技术栈等。填补 JARVIS INVENTORY 阶段中"老项目无文档可盘点"的缺口。支持全局扫描和模块深入两种模式。
---

# Jarvis Document Project — 老项目文档考古工具

JARVIS 的分支 Skill，填补黄金路径 Phase 2 INVENTORY 阶段的核心缺口：**公司有老项目但没有文档，Agent 无从盘点**。本 Skill 从代码本身逆向提取信息，让无文档的老项目也能被 JARVIS 纳入盘点。

## 在 JARVIS 中的定位

```
JARVIS 黄金路径：
  Phase 1  CLARIFY           → 明确为什么需要 JARVIS
...
**本 Skill 的产出 → JARVIS 三层技能的种子**：

| 本 Skill 的输出 | 对应的 JARVIS 技能层 | 用途 |
|---|---|---|
...
3. **只索引不倾倒** — 提取模式和结构，不是复制源码（与 JARVIS 理念一致）
4. **模式优先于日志** — 记录反复出现的模式，不是流水账（与 JARVIS 理念一致）
...
- "为什么当初选了这个方案" → 留给 JARVIS 回写
- "已知的坑和踩过的雷" → 留给 JARVIS 回写
- "被拒绝的设计方案" → 留给 JARVIS 回写
...
本 Skill 的产出（Scaffold）
  ↓ JARVIS Phase 6: 人工确认事实字段
  ↓ JARVIS Phase 7: 试点就绪
  ↓ JARVIS Phase 8: 真实工作中 START → WORK → END 回写
  ↓
Mature Knowledge（JARVIS 的组织记忆）
```
...
- 老项目缺少文档，JARVIS INVENTORY 阶段需要盘点
- 新团队成员或 AI Agent 需要快速理解项目
- 准备对老项目做改造或迁移前摸底
- 微服务架构需要梳理各服务边界和接口契约
- 为 JARVIS 的 Repo Skill / Workflow Skill 生成初始 Stub
...
扫描项目中已有的文档文件（README、docs/、*.md），作为补充信息源而非替代。
