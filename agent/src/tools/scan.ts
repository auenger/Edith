/**
 * JARVIS Scan Tool — jarvis_scan implementation
 *
 ...
 * - Repo path resolution from jarvis.yaml repos mapping
...
/** Input parameters for jarvis_scan */
...
function targetNotFoundError(target: string): ScanError {
  return {
    ...
    message: `未找到 ${target} 的配置，请检查 jarvis.yaml`,
    suggestion: `在 jarvis.yaml 的 repos 段添加 ${target} 的路径配置，或使用绝对路径作为 target。`,
  };
}
...
function pathNotFoundError(path: string): ScanError {
  return {
    code: "PATH_NOT_FOUND",
    message: `项目路径不存在: ${path}，请检查 jarvis.yaml 中的路径配置`,
    suggestion: "确认路径正确，或更新 jarvis.yaml 中对应的 path 字段。",
  };
}
...
function permissionDeniedError(path: string): ScanError {
  return {
    ...
    suggestion: "检查目录权限，或使用有读取权限的用户运行 JARVIS。",
  };
}
...
function missingParameterError(param: string): ScanError {
  return {
    ...
    suggestion: "jarvis_scan 需要 target 参数，指定要扫描的项目名或路径。",
  };
}
 ...
 * 2. If target matches a repo name in jarvis.yaml -> use mapped path
...
export function persistScanResult(
  ...
  const overviewContent = [
    ...
    "",
    "> Auto-generated by JARVIS scan at " + scannedAt,
    "",
    "## Summary",
    "",
