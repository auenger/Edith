 ...
 *
 * Executes the full EDITH pipeline: scan -> distill -> query -> route
 * using the EDITH repo itself as the test project.
...
const TEST_TARGET = "edith-repo";
const WORKSPACE = resolve("./company-edith");

// 5 test queries
const TEST_QUERIES = [
  {
    question: "edith-repo 的技术栈是什么",
    expectedContains: ["Node.js", "Python", "TypeScript"],
    description: "查询技术栈",
  },
  {
    question: "edith-repo 有哪些 API 端点",
    expectedContains: ["endpoints"],
    description: "查询 API 端点",
  },
  {
    question: "edith-repo 的数据模型是什么",
    expectedContains: ["model"],
    description: "查询数据模型",
  },
  {
    question: "edith-repo 的核心业务流程是什么",
    expectedContains: ["flow", "业务"],
    description: "查询业务流程",
  },
  {
    question: "edith-repo 的配置管理是如何实现的",
    expectedContains: ["edith.yaml", "config"],
    description: "查询配置管理",
  },
];
...
const TEST_ROUTES = [
  {
    requirement: "给 edith-repo 加一个新的扫描模式，支持增量扫描",
    expectedDecision: "quick-ref" as const,
    expectedService: "edith-repo",
    description: "功能变更路由",
  },
  {
    requirement: "修改 edith-repo 的数据模型，添加新字段到 scan result",
    expectedDecision: "deep-dive" as const,
    expectedService: "edith-repo",
    description: "Schema 变更路由",
  },
  {
    requirement: "紧急修复 edith-repo 扫描超时问题",
    expectedDecision: "quick-ref" as const,
    expectedService: "edith-repo",
    description: "故障修复路由",
  },
...
async function main() {
  console.log("=== EDITH E2E Pilot Test ===\n");
...
function generatePilotReport(pilot: PilotResult): void {
  ...
  lines.push("# EDITH Pilot Report");
  lines.push("");
  lines.push("## 基本信息");
  lines.push(`- 试点项目: EDITH Repo (自扫描)`);
  lines.push(`- 试点日期: ${now}`);
  lines.push(`- EDITH 版本: 0.1.0 (Phase 1 MVP)`);
  lines.push("");
  ...
  lines.push("");
  lines.push(`---`);
  lines.push(`*Generated by EDITH E2E Pilot at ${now}*`);
