# EDITH 可扩展性分析 — 微服务规模下的瓶颈与设计方向

> 分析日期：2026-04-27\
> 实施日期：2026-04-27\
> 背景：当公司微服务数量增多时，EDITH 在代码参与和上下文加载两方面暴露结构性瓶颈

***

## 一、两个核心问题

### 问题 1：EDITH 不应该直接参与每个微服务的代码编写

EDITH 的三层技能（Source → Repo → Workflow）本质是**路由和协调层**，它擅长的是：
...
但 EDITH **没有定义** Agent 在单个仓库内"怎么写代码"。Repo Skill 只回答"我在哪、怎么验证"，不负责"帮我实现这个功能"。
...
### 问题 2：新需求每次都走 EDITH 思考路径，导致过量上下文加载

```text
当前路径：
  新需求 → 加载 EDITH Entry Skill → 路由到 Repo Skill → 读文档 → 再开始干活
...
核心矛盾：**EDITH 是为 Agent 的全局认知设计的，但实际开发 90% 的时间只需要局部知识。**

***

## 二、EDITH 当前设计的隐含假设与缺口

### 隐含假设

EDITH 假设每个任务都需要先建立全局认知再动手。这在"新人接手老项目"场景下是对的，但在日常开发中是浪费。
...
将 EDITH 的知识分为三个加载层次，按需取用：

```text
Layer 0 — 路由表（常驻, <500 token）
  存储：<company>-edith/skills/<company-edith>/routing-table.md
...
Layer 1 — 速查卡（按需, <2000 token）
  存储：<company>-edith/skills/<repo-skill>/quick-ref.md
...
Layer 2 — 蒸馏片段（深入, <1000 token/片段）
  存储：<company>-edith/distillates/<repo-name>/01-topic.md
...
速查卡                   ~5%     只留：验证命令 + 约束 + 易错点
  ...
  ↓ distillator --edith-mode=repo-skill（现有）
EDITH Repo Skill 蒸馏版  ~15%   Agent 操作仓库需要的信息
```

**--quick-ref 与 --edith-mode 正交组合**：
...
| EDITH Repo Skill | ~15% | Agent 操作仓库  | 接口契约、数据模型、构建命令 | 环境搭建、迁移历史、所有 TODO  |
...
对于标准化微服务，用模板替代 EDITH 的直接参与：

```text
新增微服务时：
  1. EDITH 路由到正确的项目模板（Workflow Skill）
  2. 模板生成项目骨架（代码脚手架）
  3. Agent 只需填充业务逻辑
...
| 文件                   | 原因                |
| -------------------- | ----------------- |
| `SKILL.md`（主 EDITH） | 核心黄金路径不变，所有扩展为增量式 |
...
distillator --quick-ref repo-inventory.md + existing repo skills
  → skills/<company-edith>/routing-table.md
```
...
```bash
distillator --quick-ref --edith-mode=repo-skill distillates/<repo>/
...
2. distillator --edith-mode=repo-skill
   → 蒸馏版按主题拆分（Layer 2 就绪）

3. distillator --quick-ref --edith-mode=repo-skill
   → 每个服务的速查卡（Layer 1 就绪）
...
4. ~~需求路由前置分析~~ — ✅ `edith-skills/requirement-router/SKILL.md`
...
* `edith-skills/requirement-router/SKILL.md` — Skill 定义（已创建）

***

## 十、关键纪律

这个优化方向与 EDITH 原有理念完全一致，只是将其延伸到上下文管理领域：

| EDITH 原有原则 | 在上下文管理中的对应             |
| ----------- | ---------------------- |
| 索引不倾倒       | 按需加载，不全量推送             |
...
**不是否定 EDITH，而是让 EDITH 在规模增长时依然轻量可用。**
