...
┌─── 生产模式（自主工作）────────────────────────────────────────┐
│                                                              │
│  edith scan user-service                                    │
│    → 调用 document-project Skill                             │
│    → 扫描代码 → 产出 docs/                                    │
│                                                              │
│  edith distill user-service                                 │
│    → 调用 distillator Skill                                  │
│    → 产出 Layer 1 速查卡 + Layer 2 蒸馏片段                    │
│                                                              │
│  edith build-routing-table                                  │
│    → 从所有服务提取信息                                        │
│    → 产出 Layer 0 路由表                                      │
│                                                              │
│  edith refresh user-service                                 │
│    → 检测代码变更                                             │
│    → 增量更新知识产物                                          │
│                                                              │
└──────────────────────────────────────────────────────────────┘
...
```typescript
// extensions/edith.ts — 核心路由逻辑（伪代码）
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      edith_mode: Type.Optional(Type.String()),
    }),
    handler: async (args) => {
  ...
  pi.registerTool({
    ...
    handler: async (args) => {
      // 三层加载策略
      const routingTable = readFile("skills/company-edith/routing-table.md");
...
行为规则：
  - 用户说"扫描"/"分析项目" → 调用 edith_scan
  - 用户说"蒸馏"/"压缩文档" → 调用 edith_distill
  - 用户说"路由"/"这个需求" → 调用 edith_route
  - 用户说"查询"/"XX是什么" → 调用 edith_query
...
│                                                     │
├─────────────────────────────────────────────────────┤
│  ./company-edith  │  3 services  │  12 artifacts   │
└─────────────────────────────────────────────────────┘
```
...
## 三、EDITH Board — Web 看板

### 3.1 定位

EDITH Agent 解决**知识的生产和消费**，EDITH Board 解决**知识的可视化和管理**。

```text
┌──────────────────────────────────────────────────────┐
│                                                      │
│   EDITH Agent          EDITH Board                 │
│   (终端)                (浏览器)                      │
│                                                      │
│   知识生产 ──────┐      ┌────── 知识可视化            │
│   知识查询       │      │      知识管理               │
...
│  ┌─ 文件树 ──────────┐  ┌─ 预览 ──────────────────────────────┐   │
│  │                   │  │                                     │   │
│  │  📁 company-edith│  │  📄 order-service / quick-ref.md    │   │
│  │  ├── 📁 skills    │  │  ─────────────────────────────      │   │
│  │  │   ├── 📁 main  │  │                                     │   │
│  │  │   │   └── 📄 r │  │  ## Verify                          │   │
│  │  │   ├── 📁 order │  │  ```bash                            │   │
...
**关键设计：Board 只读，不修改 Agent 产出物。** 知识的生产唯一入口是 EDITH Agent。

***

## 四、EDITH Artifacts — 开放产出物

### 4.1 产出物格式

```text
company-edith/                     ← 知识仓库（单一真相源）
├── skills/
│   ├── company-edith/
...
| EDITH Extension | 编译后分发    | 中（可逆向但成本高）    |
| EDITH Skills    | 核心逻辑不暴露  | 高（知识提取质量难复制）  |
...
Week 1:
  ✅ pi SDK 环境搭建 + 项目结构
  ✅ edith.ts Extension 骨架
  ✅ edith_scan 工具实现（对接 document-project Skill）
  ✅ edith.yaml 配置管理

Week 2:
  ✅ edith_distill 工具实现（对接 distillator Skill）
  ✅ edith_query 工具实现（三层加载查询）
  ✅ edith_route 工具实现（对接 requirement-router Skill）
  ✅ TUI 主题定制（EDITH 品牌化）
...
SKILL.md（主 EDITH）    ──→    EDITH Agent 的核心 Skill
document-project Skill   ──→    edith_scan 工具的内部实现
distillator Skill        ──→    edith_distill 工具的内部实现
requirement-router Skill ──→    edith_route 工具的内部实现

三层加载策略             ──→    edith_query 的查询逻辑
