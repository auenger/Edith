...
or glob patterns. Skips node_modules, .git, __pycache__, .venv.
...
QUICK_REF_SOURCE_RATIO = 20  # quick-ref targets ~5% = 1/20 of source
...
def analyze(inputs: list[str], output_path: str | None = None) -> None:
    ...
    result = {
        ...
        "quick_ref_prediction": {
            "target_tokens": QUICK_REF_TARGET_TOKENS,
            "estimated_quick_ref_tokens": max(
                total_tokens // QUICK_REF_SOURCE_RATIO, 200
            ),
            "feasible": (
                max(total_tokens // QUICK_REF_SOURCE_RATIO, 200)
