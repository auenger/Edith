# Edith Skills — 老项目文档考古工具集

EDITH 的分支 Skill，用于从无文档的老项目中逆向生成 AI 可消费的文档。填补 EDITH 黄金路径 Phase 2 INVENTORY 阶段中"老项目无文档可盘点"的缺口。
...
**功能**：将大文档集无损压缩为 token 高效的精简版。是 EDITH "索引不倾倒"原则的工具化实现。
...
**功能**：判断用户需求是否需要加载 EDITH 上下文，以及加载哪一层。是三层加载策略的决策入口。
...
| 项    | 说明                                    |
| ---- | ------------------------------------- |
| 定位   | EDITH 黄金路径 Phase 2 INVENTORY 的代码考古工具 |
| 依赖   | 无外部依赖，直接使用                            |
| 语言   | 中文为主                                  |
| 配置   | 启动时交互确认                               |
