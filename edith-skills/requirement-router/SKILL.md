---
name: edith-requirement-router
description: EDITH 分支 Skill。需求路由前置分析，判断用户需求是否需要加载 EDITH 上下文以及加载哪一层。是三层加载策略（Layer 0/1/2）的决策入口，防止不必要的上下文加载。
---

# Edith Requirement Router — 需求路由前置分析

EDITH 的分支 Skill，是三层加载策略的**决策入口**。在 Agent 开始工作前，用最小成本（仅读 Layer 0 路由表，<500 token）判断需求是否需要加载额外上下文、加载哪一层，**防止不必要的上下文浪费**。
