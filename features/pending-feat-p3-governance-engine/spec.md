...
### General Checklist
- [ ] 知识生命周期状态机实现 (scaffold / reviewed / mature / stale)
- [ ] frontmatter.ts 扩展 lifecycle / governance / quality 字段
- [ ] 矛盾检测引擎（复用 edit-detector.ts）
- [ ] 健康度评分模型（四维）
- [ ] 治理 JSON 数据文件生成（.edith/governance/）
- [ ] edith governance CLI 命令（status / review / refresh）
- [ ] edith.yaml 新增 governance 配置块
- [ ] 向后兼容：禁用 governance 时默认行为不变
