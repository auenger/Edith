...
配置 EDITH Agent 的打包和分发机制：Pi Package 打包配置、npm/git 分发渠道、安装脚本、版本管理。确保用户可以通过简单命令安装和更新 EDITH。

## OUT Scope

本功能 **不包含** 以下内容：
- 自动更新机制（不实现后台自动检查新版本）
- 多版本共存（不支持同时安装多个 EDITH 版本）
...
1. **一键安装** — 用户通过 `pi install edith` 或 `npm install -g @edith/agent` 安装
2. **首次引导** — 安装后首次运行自动检测配置，引导初始化
3. **版本可查** — `edith --version` 查看当前版本

## Context Analysis

### Reference Code
- `EDITH-PRODUCT-DESIGN.md` § 6 技术栈 - 分发行
...
> 6. Pi Package 和 npm package 完全共存：Pi 包就是 npm 包，通过 `pi install npm:@edith/agent` 安装。
>
> **实施策略调整：**
...
├── package.json             # npm 包配置（bin 字段注册 edith 命令）
├── bin/
│   └── edith.ts            # CLI 入口（解析 --version / --init / 默认启动）
├── src/
...
│       └── system-prompt.md
├── scripts/
│   └── post-install.ts      # 安装后脚本（检测 edith.yaml）
├── templates/               # 配置模板
│   └── edith.yaml.template
└── CHANGELOG.md             # 版本变更记录
```
...
description: "EDITH - AI Knowledge Infrastructure Agent"
entry: bin/edith.ts     # CLI 入口

extensions:
  - name: edith-extension
...
```text
post-install.ts:
  1. 检测当前目录是否存在 edith.yaml
  2. 如果存在 → 输出 "检测到已有配置，直接运行 edith 开始使用"
  3. 如果不存在 → 输出 "未检测到配置文件，运行 edith --init 开始初始化"
...
```text
bin/edith.ts:
  - edith --version → 读取 package.json version 并输出
  - edith --init    → 触发初始化向导（交互式生成 edith.yaml）
  - edith           → 正常启动 Agent
```
...
When 执行 `pi install edith`
Then EDITH 安装成功，无报错
And `edith` 命令已注册到 PATH
And `edith --version` 输出正确的版本号
```
...
```gherkin
Given EDITH 已通过 pi install 安装成功
And 当前目录不存在 edith.yaml 配置文件
When 用户执行 `edith`
Then 输出 "未检测到配置文件 (edith.yaml)"
And 提示 "运行 edith --init 开始初始化"
And 不自动创建配置文件
And 不直接启动 Agent
```
...
```gherkin
Given EDITH 已通过 pi install 安装成功
And 当前目录存在有效的 edith.yaml 配置文件
When 用户执行 `edith`
Then EDITH Agent 正常启动
And 加载 edith.yaml 中的配置（LLM provider、repos 等）
And 显示 EDITH 品牌化启动界面
```

**Scenario 6: 版本查询**
```gherkin
Given EDITH 已安装
When 执行 `edith --version`
Then 输出语义化版本号（如 "edith v0.1.0"）
And 版本号与 pi-package.yaml 和 package.json 中一致
```

**Scenario 7: 从旧版本升级 — 保留配置**
```gherkin
Given 用户已安装 EDITH v0.1.0 且有 edith.yaml 配置文件
When 用户执行 `pi install edith` 安装 v0.2.0
Then EDITH 升级到 v0.2.0
And 原有的 edith.yaml 配置文件保持不变
And `edith --version` 输出 v0.2.0
And 生成的知识产物（distillates 等）保持不变
```

**Scenario 8: 初始化向导**
```gherkin
Given EDITH 已安装但当前目录无 edith.yaml
When 执行 `edith --init`
Then 启动交互式初始化向导
And 依次引导配置：LLM provider、workspace 路径、repos 列表
And 生成有效的 edith.yaml 文件
And 后续执行 `edith` 可正常启动
```
