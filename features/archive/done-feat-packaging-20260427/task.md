...
- [x] T1.2 更新 `package.json`：name=@jarvis/agent、bin 字段注册 `jarvis` 命令、scripts.postinstall、files 白名单
- [x] T1.3 创建 `CHANGELOG.md`：初始版本 0.1.0 条目
- [x] T1.4 验证 package.json 中版本号一致性（0.1.0）

### Phase 2: CLI 入口 (Scenario 4, 5, 6, 8)

- [x] T2.1 创建 `src/bin/jarvis.ts`：解析命令行参数
- [x] T2.2 实现 `--version` 命令：读取 package.json version 并输出 → 验证通过：`jarvis v0.1.0`
- [x] T2.3 实现 `--init` 命令：委托给 config.ts 的 initConfigWizard
- [x] T2.4 实现默认启动：检测 jarvis.yaml 是否存在
  - 存在 → 加载配置，启动 Agent
  - 不存在 → 提示运行 `jarvis --init`，不自动创建
- [x] T2.5 测试 Scenario 4：无 jarvis.yaml 时执行 `jarvis`，验证引导提示 ✓
- [x] T2.6 测试 Scenario 5：有 jarvis.yaml 时执行 `jarvis`，验证正常启动 ✓（委托给 agent-startup）
- [x] T2.7 测试 Scenario 6：执行 `jarvis --version`，验证版本号输出 ✓
- [x] T2.8 测试 Scenario 8：执行 `jarvis --init`，验证向导流程和配置生成 ✓（委托给 initConfigWizard）

### Phase 3: 安装后脚本 (Scenario 2, 3)

- [x] T3.1 创建 `src/scripts/post-install.ts`：检测 jarvis.yaml 存在性
- [x] T3.2 实现逻辑：存在 → 输出 "检测到已有配置"；不存在 → 输出 "运行 jarvis --init"
- [x] T3.3 创建 `templates/jarvis.yaml.template`：包含所有可配置字段和注释说明
...
- [x] T4.3 验证 dist/ 内容：bin/jarvis.js、scripts/post-install.js、agent-startup.js、index.js ✓
- [x] T4.4 测试 Scenario 1：完整编译流程验证 ✓

### Phase 5: 升级验证 (Scenario 7)

- [ ] T5.1 模拟已安装 v0.1.0 状态（jarvis.yaml + 知识产物）— 需实际 npm publish 后验证
- [ ] T5.2 执行升级到 v0.2.0 — 需实际 npm publish 后验证
- [ ] T5.3 验证 jarvis.yaml 配置保持不变 — 需实际升级后验证
- [ ] T5.4 验证知识产物保持不变 — 需实际升级后验证
- [ ] T5.5 验证 `jarvis --version` 输出 v0.2.0 — 需实际升级后验证
