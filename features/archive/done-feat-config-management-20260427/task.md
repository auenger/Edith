...
- [x] 阅读 `JARVIS-PRODUCT-DESIGN.md` § 2.6 配置模型完整 YAML 示例
- [x] 确认 TypeScript 严格模式下 interface 的类型推断行为

### 2. TypeScript Interface 定义（Scenario 1）
- [x] 定义 `JarvisConfig` 接口 — 所有字段及类型 — Scenario 1
...
- [x] 文件路径搜索逻辑：当前目录 → 逐级向上搜索 jarvis.yaml — Scenario 11
- [x] 文件不存在时返回明确错误 + 提示运行 jarvis-init — Scenario 5
...
- [x] 实现 `applyDefaults(config: Partial<JarvisConfig>)` 函数 — Scenario 7
- [x] repos 默认为空数组 []
- [x] 仅填充未提供的字段，不覆盖已有值
...
- [x] 步骤 4：输入 workspace root 路径（默认 ./company-jarvis） — Scenario 3
- [x] 步骤 5：选择 language（zh / en） — Scenario 3
- [x] 步骤 6：添加 repo 列表（可重复添加，输入空行结束） — Scenario 3
- [x] 步骤 7：确认并生成 jarvis.yaml — Scenario 3
- [x] Ctrl+C 中断时不生成不完整文件 — Scenario 10
- [x] 已有 jarvis.yaml 时提示覆盖确认

### 8. 导出公共 API
- [x] 导出 `loadConfig()` — 供 feat-extension-core 的 jarvis-init 命令调用
- [x] 导出 `validateConfig()` — 供 Agent 启动时调用
- [x] 导出 `initConfigWizard()` — 供 jarvis-init 命令调用
- [x] 导出 `JarvisConfig` 及所有子接口

### 9. Verification — 验收验证
- [x] Scenario 1: 正确 YAML 解析为 JarvisConfig 对象
- [x] Scenario 2: 缺少 llm.provider 报错并阻止启动
- [x] Scenario 3: jarvis-init 完整向导流程生成 jarvis.yaml
- [x] Scenario 4: 无效 YAML 语法报错含行号
- [x] Scenario 5: 配置文件不存在时提示 jarvis-init
...
- [x] Scenario 11: 上级目录搜索找到 jarvis.yaml
...
| 2026-04-27 | Implementation complete | All 9 task groups implemented. TypeScript compiles cleanly. Files: config.ts (full rewrite), index.ts (updated imports), jarvis.yaml (updated with agent section) |
