...
- [x] 阅读 `JARVIS-PRODUCT-DESIGN.md` § 2.3 Extension 核心设计伪代码
- [x] 阅读 `jarvis-skills/INTEGRATION.md` 了解 Skill 加载协议

### 2. Tool Registration — 工具注册（Scenario 1, 2, 7）
- [x] 定义 jarvis_scan 工具的 TypeBox 参数 schema（name, path, options） — Scenario 1
- [x] 定义 jarvis_distill 工具的 TypeBox 参数 schema（source, options） — Scenario 1
- [x] 定义 jarvis_route 工具的 TypeBox 参数 schema（requirement） — Scenario 1
- [x] 定义 jarvis_query 工具的 TypeBox 参数 schema（question, context） — Scenario 1
...
- [x] 注册 `jarvis-init` 命令，handler 为 stub（显示 "not implemented yet"） — Scenario 11
- [x] 注册 `jarvis-status` 命令，handler 返回当前状态概览 — Scenario 12
