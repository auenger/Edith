...
设计并调优 EDITH Agent 的 System Prompt。定义行为规则（自然语言→工具触发映射）、知识来源标注格式、错误处理话术、边界情况处理策略。

## OUT Scope

本功能 **不包含** 以下内容：
- 工具逻辑实现（edith_scan / edith_distill / edith_route / edith_query 的内部实现）
...
### Reference Code
- `EDITH-PRODUCT-DESIGN.md` § 2.4 System Prompt（初始版本）
...
```
## 1. 角色定义
你是 EDITH，组织的 AI 知识基础设施。你的职责是帮助团队从代码中提取、管理、消费知识。
...
| 用户意图关键词              | 触发工具       |
|---------------------------|---------------|
| 扫描、分析代码、项目结构      | edith_scan   |
| 蒸馏、知识提取、生成文档      | edith_distill|
| 查询、某个接口、数据模型      | edith_query  |
| 路由、这个需求归谁、哪个服务   | edith_route  |
...
```gherkin
Given EDITH Agent 已启动且 System Prompt 已加载
When 用户说 "分析一下订单服务的代码"
Then Agent 识别为扫描意图（"分析" + "代码" → scan）
And 调用 edith_scan 工具
And 不向用户暴露 edith_scan 这个工具名
```
...
And edith_query 返回了 distillates/order-service/02-api-contracts.md 中的结果
Then Agent 组织自然语言回答
...
And Agent 先执行 edith_scan，等待完成
And 扫描成功后自动执行 edith_distill
And 全程向用户报告进度："正在扫描 order-service... 扫描完成，开始蒸馏..."
```

**Scenario 6: 工具不可用 — 优雅降级**
```gherkin
Given EDITH Agent 已启动但知识库为空（未扫描过任何服务）
...
And 触发 edith_scan，目标为 payment-service
And 使用与用户输入语言一致的中文回复
```

**Scenario 8: 长对话上下文管理**
```gherkin
Given 用户与 EDITH Agent 已对话超过 10 轮
