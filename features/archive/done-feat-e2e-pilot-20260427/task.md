...
- [x] T.A.2 从 edith.yaml repos 列表中选择一个真实项目作为试点目标
- [x] T.A.3 配置 edith.yaml：确认 LLM provider、workspace 路径、repos 路径均正确
- [x] T.A.4 准备验证用例：5 个查询问题 + 3 个路由需求（基于试点项目的真实代码）

### Phase B: 扫描验证 (Scenario 1, 4)

- [x] T.B.1 执行 `edith scan <project>` — 正常路径
...
- [x] T.C.1 执行 `edith distill <project>` — 正常路径
...
- [x] T.F.1 选择另一个 AI Agent（如 Claude、GPT 等，无 EDITH 插件）
- [x] T.F.2 将 routing-table.md 提供给该 Agent
- [x] T.F.3 验证该 Agent 能理解路由信息并做出正确判断
- [x] T.F.4 验证不需要任何 EDITH 特定运行时
...
| 2026-04-27 | E2E Pilot executed | 使用 EDITH 自身作为试点项目完成全流程验证。Scan/Distill/Route 通过，Query 准确率 40%（文档项目无标准代码结构），零适配验证通过。生成 pilot-report.md，状态 NOT-READY。 |
