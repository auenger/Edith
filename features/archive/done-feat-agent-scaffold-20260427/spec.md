...
搭建 EDITH Agent 的项目骨架：初始化 TypeScript 项目、集成 pi SDK (`@mariozechner/pi-coding-agent`)、建立 Extension 骨架结构、配置 edith.yaml 解析入口。
...
- edith.yaml 最小解析（读取配置并打印）
- `npm start` 可启动 Agent 并显示欢迎信息

**OUT:**
- 不实现任何 edith_* 工具的业务逻辑（feat-extension-core 负责）
- 不实现 TUI 品牌化（feat-tui-branding 负责）
- 不实现配置验证 / 热加载（feat-config-management 负责）
- 不实现 edith-init 交互向导（feat-config-management 负责）
- 不 fork pi SDK，仅作为 npm 依赖引入

## User Value Points

1. **可运行的 Agent 空壳** — 用户可以在终端启动 EDITH Agent（即使还没有实际功能），看到欢迎信息和交互式提示符
2. **pi SDK 集成验证** — Agent Loop、TUI 基础设施就绪，后续 feature 只需注册工具即可扩展功能

## Context Analysis

### Reference Code
- `edith-skills/` — 三个现有 Skill 的目录结构作为项目组织参考
- `edith-skills/document-project/SKILL.md` — Skill 定义规范
- `edith-skills/distillator/scripts/analyze_sources.py` — 唯一 Python 脚本参考
- `SKILL.md` — 黄金路径定义

### Related Documents
- `EDITH-PRODUCT-DESIGN.md` § 2.1 架构图、§ 2.3 Extension 核心设计（伪代码）
- `EDITH-PRODUCT-DESIGN.md` § 2.6 配置模型（edith.yaml 完整示例）
...
### User Story
作为 EDITH 开发者，我需要一个可运行的 Agent 项目骨架，以便在此基础上逐步添加 edith_scan / edith_distill 等工具。
...
And 终端显示 "EDITH" 或 "edith" 相关欢迎文字
And 进入交互式命令行等待输入
```

**Scenario 2: edith.yaml 配置加载**
```gherkin
Given edith.yaml 文件存在且包含 llm.provider: "anthropic" 和 workspace.root: "./company-edith"
When Agent 启动
Then 配置被解析为 TypeScript 对象
And workspace.root 值为 "./company-edith"
And repos 为空数组
```
...
**Scenario 4: edith.yaml 不存在时的处理**
```gherkin
Given 当前目录下不存在 edith.yaml 文件
When 执行 `npm start`
Then 显示友好错误 "未找到 edith.yaml 配置文件，请先创建配置"
And Agent 不崩溃（退出码非 0 但无 uncaught exception）
```

**Scenario 5: edith.yaml 格式错误时的处理**
```gherkin
Given edith.yaml 存在但包含无效 YAML 语法（如缩进错误）
When 执行 `npm start`
