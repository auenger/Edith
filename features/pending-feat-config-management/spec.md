# Feature: feat-config-management jarvis.yaml 配置管理

## Basic Information
- **ID**: feat-config-management
- **Name**: jarvis.yaml 配置管理
- **Priority**: 90
- **Size**: S
...
实现 jarvis.yaml 的完整配置管理：解析、验证、类型定义、默认值填充。支持 LLM provider 配置、workspace 路径、repo 列表、token budget 等配置项。提供 `jarvis-init` 交互式配置向导。

## OUT Scope（本 feature 不做）

- **热加载（hot reload）** — 运行时监听 jarvis.yaml 文件变更并自动重载不在本 feature 范围内。配置仅在 Agent 启动时读取一次
- **配置加密** — api_key 等敏感字段不做加密存储，仅标记为可选字段
- **多环境配置** — 不支持 jarvis.dev.yaml / jarvis.prod.yaml 等环境拆分
...
1. **零代码定制** — 用户通过 jarvis.yaml 定义 LLM、workspace、repos，无需改代码
2. **配置验证** — 启动时校验配置完整性，错误时给出明确提示
3. **交互式初始化** — jarvis-init 向导引导新用户生成配置文件，降低上手门槛
4. **合理默认值** — 可选字段有合理默认值，最小配置只需 LLM provider + workspace

## Context Analysis

### Reference Code
- `JARVIS-PRODUCT-DESIGN.md` § 2.6 配置模型（完整 YAML 示例）
...
- feat-extension-core（并行）— jarvis-init 命令 stub 由 Extension 层注册，本 feature 提供实际实现

## Technical Solution

### JarvisConfig TypeScript Interface
...
- **YAML 解析**: 使用 `js-yaml` 库解析 jarvis.yaml
- **验证**: 自定义验证函数检查必填字段和类型
- **默认值**: 对可选字段（agent.auto_refresh, agent.refresh_interval, repos[].stack）提供默认值
- **jarvis-init 向导**: 使用 `@inquirer/prompts` 或类似库实现交互式配置生成
...
### Scenario 1: 配置解析成功
```gherkin
Given jarvis.yaml 存在且格式正确
When Agent 启动
Then 配置被解析为类型安全的 JarvisConfig 对象
And 所有字段有合理默认值
```

### Scenario 2: 配置校验 — 缺少必填字段
```gherkin
Given jarvis.yaml 缺少 llm.provider 字段
When Agent 启动
Then 显示明确错误 "配置校验失败: llm.provider 是必填项"
And Agent 不启动
```

### Scenario 3: jarvis-init 交互向导
```gherkin
Given 用户执行 jarvis-init 命令
Then 进入交互式配置向导
And 依次提示 LLM provider、model、workspace 路径、repo 列表
And 最终生成 jarvis.yaml
```

### Scenario 4: YAML 语法错误（sad-path）
```gherkin
Given jarvis.yaml 包含无效 YAML 语法（如缩进错误、未闭合引号）
When Agent 尝试加载配置
...
```gherkin
Given 当前目录及上级目录均不存在 jarvis.yaml
When Agent 启动
Then 显示提示 "未找到 jarvis.yaml，运行 jarvis-init 生成配置文件"
And Agent 不启动
```

### Scenario 6: 无效枚举值（sad-path）
```gherkin
Given jarvis.yaml 中 workspace.language 为 "fr"
When Agent 尝试加载配置
...
```gherkin
Given jarvis.yaml 仅包含 llm 和 workspace 段
When Agent 加载配置
...
### Scenario 8: 环境变量引用
```gherkin
Given jarvis.yaml 中 api_key 值为 "${OPENAI_API_KEY}"
...
### Scenario 10: jarvis-init 向导中断
```gherkin
Given 用户正在执行 jarvis-init 交互向导
When 用户在任意步骤按 Ctrl+C 中断
Then 不生成不完整的 jarvis.yaml 文件
And 显示提示 "配置向导已取消"
```

### Scenario 11: 配置文件路径搜索
```gherkin
Given jarvis.yaml 存在于上级目录 /project/jarvis.yaml
And Agent 在 /project/subdir 启动
Then Agent 向上搜索并找到 /project/jarvis.yaml
And 配置正常加载
```

### General Checklist
- [ ] JarvisConfig TypeScript 接口定义完整
- [ ] YAML 解析使用 js-yaml
